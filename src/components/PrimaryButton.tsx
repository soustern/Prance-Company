...
const PrimaryButton = ({text, children, className}: PrimaryButtonProps): JSX.Element => {
    ...
    return (
        <motion.button whileTap={{scale: 0.9}} transition={{duration: 0.05, type: "spring", stiffness: 500, damping: 30}} className={`text-slate-200 px-8 py-4 w-full cursor-pointer relative z-0 overflow-hidden rounded-4xl font-medium text-2xl md:text-xl md:px-12 md:py-3 ${className}`}>
