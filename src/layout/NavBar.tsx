                ...
                <div className="flex items-center justify-between">
                    ...
                    <div id="hamburger">
                        <motion.button whileTap={{scale: 0.8}} transition={{duration: 0.05, type: "spring", stiffness: 500, damping: 30}} className="cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
                            {
                                isOpen ? <i className="fa-solid fa-xmark text-slate-50 text-5xl"></i> : <i className="fa-solid fa-bars text-slate-50 text-5xl"></i>
                            }
                        </motion.button>
                    </div>
                </div>
                <AnimatePresence>{isOpen && <motion.div id="menu" initial={{height: 0}} animate={{height: "100vh"}} transition={{duration: 0.15, ease: "easeInOut"}} className="bg-[var(--color-bg-primary)] px-8 py-8 w-full overflow-hidden" exit={{height: 0}}>
                    <ul className="w-full flex flex-col items-start gap-8">
                        {
                            navOptions.map((option, index) => {
                                return (
                                    <li className="py-8 w-full relative" onClick={() => setIsOpen(false)}>
                                        <motion.div className="flex items-center justify-between" initial={{translateY: 10, opacity: 0}} animate={{translateY: 0, opacity: 1}} transition={{duration: 0.2, delay: index * 0.08}}>
                                            <p className="text-slate-50 text-3xl font-medium">{option}</p>
